<template>
  <div class="w-full text-center relative z-10 px-4 md:px-0">
    ...
    <Transition name="fade">
      <div v-if="selectedItem" class="fixed inset-0 z-[99999] flex items-center justify-center pt-24 pb-8 px-4 text-left overflow-y-auto">
        <div class="fixed inset-0 bg-[#004732]/80 backdrop-blur-sm" @click="closeModal"></div>

        <div @click.stop class="bg-white rounded-3xl p-6 md:p-10 max-w-2xl w-full shadow-2xl relative z-10 overflow-y-auto max-h-[80vh] border-t-4 border-[#FFBD59] flex flex-col my-auto">
